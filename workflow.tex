...
% Variables
\input{out/mingle_winkel_diag_var.tex}
\input{out/hemi_anal_var.tex}
...
\lstinputlisting[label=ptx_laz,language=bash,caption={The processing chain used to convert Leica Cyclone PTX files to LAZ files, using PDAL, POSIX shell scripting, and common UNIX utilities. The \texttt{ptx\_laz.json} JSON pipeline is shown in \autoref{ptx_laz_json}.}]{out/ptx_laz.txt}
\end{minipage}

\vspace{\baselineskip}
\begin{minipage}{\linewidth}
	\lstinputlisting[label=ptx_laz_json,language=json,caption={The JSON pipeline used in \autoref{ptx_laz} to convert PTX files to LAZ files, and applying a rotation matrix.}]{out/ptx_laz_json.txt}
\end{minipage}
